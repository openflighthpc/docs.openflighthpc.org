...
=== "Launch Instructions"
    ...
    1. Launch the cluster (replacing `mycluster1` with your desired cluster name)
        ```bash
        openstack stack create -t cfd-jumpstart.yml \
    ...
    It will take about 15 minutes for the cluster to launch and fully configure itself to be ready for usage. Progress of the application can be verified as complete when `flight profile list` on `gateway1` shows all nodes with a status of `complete`.
...
=== "Usage Instructions"
    ...
    OpenFOAM 22.12 is installed under `/opt/apps/` and is therefore shared across all the nodes in the cluster.

    Paraview and OpenMPI have been installed via system packages. The `gateway1` node is the only one with Paraview installed and OpenMPI is installed on every compute node. 
